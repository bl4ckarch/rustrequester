...
fn thread_function(stop_flag: Arc<AtomicBool>, shared_data: Arc<Mutex<SharedData>>, client: Client, method: Method, url: String, headers: HeaderMap, body: String, num_requests: usize) {
...
fn display_speed(shared_data: Arc<Mutex<SharedData>>, start: Instant, stop_flag: Arc<AtomicBool>) {
    loop {
        ...
        let total_requests = shared_data.lock().unwrap().request_count;
...
/*le code c'est la doc lis et comprend*/
...
@https://github.com/bl4ckarch
    ...
    let num_requests: usize = matches.value_of("requests").unwrap().parse().expect("Invalid number for requests");
